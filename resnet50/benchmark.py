#Credits: https://github.com/vwxyzjn/cleanrl/blob/master/cleanrl_utils/benchmark.py
# this is copied because installing CleanRL produces versioning conflicts with jax, and learned_optimizaiton
...
if __name__ == "__main__":
    ...
    for seed in range(args.start_seed, args.start_seed + args.num_seeds):
        commands += [" ".join([args.command, "--seed", str(args.start_seed)])]
    ...
    if args.workers > 0:
        ...
        executor = ThreadPoolExecutor(max_workers=args.workers, thread_name_prefix="resnet-cifar10-")
        for command in commands:
